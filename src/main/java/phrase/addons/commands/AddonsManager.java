...
public class AddonsManager implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command,
        ...
        if(strings[0].equalsIgnoreCase("reload")) {
            ...
            String message = UtilHexColor.colorize(hex, Plugin.getInstance().getConfig().getString("message.prefix")) + color(Plugin.getInstance().getConfig().getString("message.command.reload.succesful"));
            commandSender.sendMessage(message);
            return true;
        }
